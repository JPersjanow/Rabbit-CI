...
kanban_model = api.model('Kanban Model', {
...
class KanbansAll(Resource):
    ...
    def get(self):
        ...
        try:
            all_kanbans_info_list = kanban_finder.return_all_kanabans_info(kanbans_directory=config.kanbans_directory)
    ...
    def post(self):
        ...
        try:
            new_kanban_dir, new_kanban_id= kanban_creator.create_new_kanban_folder(kanbans_directory=config.kanbans_directory)
        ...
        try:
                config_tree = kanban_creator.create_xml_tree_for_kanban_config(kanban_name=api.payload['name'], kanban_id=new_kanban_id, description=api.payload['description'])
                kanban_creator.create_new_kanban_config(new_kanban_directory=new_kanban_dir, config_xml_tree=config_tree)
        except Exception as e:
            os.rmdir(new_kanban_dir)
            return {"response": "Failed while creating config.xml file, deleting kanban.", "exception": str(e)}, 500

        return {"response": f"New kanban board with id {new_kanban_id} under {new_kanban_dir} created"}, 201
...
class KanbanSingle(Resource):
    ...
    def get(self, kanban_id):
        ...
        if kanban_found:
            kanban_info_list = kanban_finder.return_kanban_info(kanban_directory=kanban_directory)
